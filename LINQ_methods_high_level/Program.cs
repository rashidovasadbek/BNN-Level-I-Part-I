...
//group Join
var filters = customers.GroupJoin(orders,
...
{
    Console.WriteLine("Customer" + filter.customer.FirstName + " " + filter.customer.LastName);
    foreach (var customerOrder in filter.order)
    {
        Console.WriteLine($"Id - {customerOrder.Id} {customerOrder.Amount} {customerOrder.CustomerId} {customerOrder.OrderDate}");
    }
}*/
...
/*foreach (var group in groupby)
{
    ...
    foreach (var student in group)
        Console.WriteLine($"FirstName:{student.FirstName}, LastName:{student.LastName}, Country:{student.Country}");
}*/



var result = customers.Where(customer => customer.Country == Country.UK);
/*foreach(var item in result)
{
    Console.WriteLine($"Id:{item.Id}, FirstName:{item.FirstName}, LastName:{item.LastName}, Country{item.Country}");
}*/




var filtersLastMonth = customers.GroupJoin(orders,
    customer => customer.Id,
    order => order.CustomerId,
        (customer, order) => new { customer, order }
...
{
  order = orderbyCustomer.order.Where(order => (DateTime.Now - order.OrderDate).TotalDays < 30)
});

/*foreach (var filter in filtersLastMonth)
{
    Console.WriteLine("Customer" + filter.customer.FirstName + " " + filter.customer.LastName);
    foreach (var customerOrder in filter.order)
    {
...
var groupbymoney = customers.GroupJoin(orders,
    customer => customer.Id,
    order => order.CustomerId,
        (customer, order) => new { customer, order }
...
{
    order = orderbyCustomer.order.Where(order => order.Amount > 5000)
}).Where(orderByCustomer => orderByCustomer.order.Any()).ToList();

/*foreach (var filter in groupbymoney)
{
    Console.WriteLine("Customer" + filter.customer.FirstName + " " + filter.customer.LastName);
    foreach (var customerOrder in filter.order)
    {
