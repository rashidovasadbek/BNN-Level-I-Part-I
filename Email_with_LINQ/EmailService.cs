...
public class EmailService 
{
    ...
    //emaillarni sender bo'yicha topib, sanasi kamayish tartibida saralab,
    //pagination bilan qaytarsin
    public IOrderedEnumerable<Email> GetBySender(string senderEmailAddress, int pageSize, int pageToken)
    {
      if(emails.Any(email => email.SenderAddress != null || email.SenderAddress == senderEmailAddress))
        {
            var response = emails.Skip((pageToken - 1) * pageSize).Take(pageSize).ToList();
            var sortresponse = response.OrderByDescending(email => email.Date);
            return sortresponse;
        }
        return null;
    }

    //emaillarni receiver bo'yicha topib, sanasi kamayish tartibida salarab,
    //pagination bilan qaytarsin
    public IOrderedEnumerable<Email> GetByReceiver(string receiverEmailAddress,int pageSize,int pageToken)
    {
        if (emails.Any(email => email.ReceiverAddress != null || email.ReceiverAddress == receiverEmailAddress))
        {
            var response = emails.Skip((pageToken - 1) * pageSize).Take(pageSize).ToList();
            var sortresponse = response.OrderByDescending(email => email.Date);
            return sortresponse;
    ...
    //emaillarni filter model bo'yicha filter qilib qaytaradi 
    //bunda filterdagi hamma propertylar null bo'lsa hamma object qaytadi
    public void Get(EmailFilterModel filter)
    {
        foreach (var item in emails)
        {
            var filterModel = new  EmailFilterModel(item.ReceiverAddress,item.SenderAddress,item.Date,5,1);
