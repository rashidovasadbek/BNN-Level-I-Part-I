...
{ 
    ...
    public IOrderedEnumerable<Employee> GetBySalary(int pageSize, int pageToken)
    {
        var response = employes.Skip((pageToken - 1) * pageSize).Take(pageSize).ToList();
        var sortresponse =  response.OrderByDescending(response => response.Salary);
        return sortresponse;
    }
}
